  ...
  const [bought, setBought] = useState(true);
        ...
        const purchase = async (data, index) => {
          ...
          const updateSeller = await fetch(`https://streamifibackend.fly.dev/user/sold/${seller}`, 
              ...
              body: JSON.stringify({ amount: valueAmount, musicType: "Single", otherparty: address })
          });
          ...
          const updateBuyer = await fetch(`https://streamifibackend.fly.dev/user/buy/${address}`, 
              ...
              body: JSON.stringify({ amount: valueAmount, musicType: "Single", otherparty: seller })
          });
  ...
  return (
    <div className={`grid grid-rows-2 gap-4 p-8 text-white ${sideload && "opacity-25"}`}>
      ...
      <div className="flex flex-col gap-5 mt-10">
        ...
        <div className="flex justify-between p-2 pr-8 rounded-[5px] items-center" style={{background: 'rgba(217, 217, 217, 0.11)'}}>
          {/* Left /}
